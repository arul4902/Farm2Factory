// src/components/FarmerLoginForm.js
...
const FarmerLoginForm = () => {
  ...
  const handleLoginSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:9003/api/auth/farmer/login', {
        method: 'POST',
        headers: {
  ...
  const handleForgotPassword = async (event) => {
    event.preventDefault();
    try {
      const response = await fetch('http://localhost:9003/api/auth/farmer/forgot-password', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
